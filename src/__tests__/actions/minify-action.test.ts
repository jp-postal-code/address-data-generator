...
import { createMinifyCommand } from '@/lib/commands/create-minify-command';
...
test('minify action', async () => {
  ...
  await minifyAction(join(workDir, '**/*.json'), {}, createMinifyCommand());
  ...
  expect(logOutput).toMatch('[SUCCESS] Minify');
});
